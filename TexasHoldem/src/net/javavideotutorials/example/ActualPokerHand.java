...
public class ActualPokerHand implements Comparable<ActualPokerHand>
{
  ...
  private String determineWinningDuplicates(List<Card> winningCards, int numDuplicates)
  {
    ...
    Map<Value, List<Card>> valueMap = Game.checkforDup(winningCards);
  ...
  private String determineWinningDuplicatesSuits(List<Card> winningCards, int numDuplicates)
  {
	
    Map<Suit, List<Card>> suitMap = Game.checkforDupSuit(winningCards);
    List<Card> winningDupes = new ArrayList<Card>();
    for (Entry<Suit, List<Card>> entry : suitMap.entrySet())
    {
      if (numDuplicates == 1)
      {
        if (entry.getValue().size() == 2)
